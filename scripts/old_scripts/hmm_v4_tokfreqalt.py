...
#Version 4_tokfreqalt: Use the token frequency to the determine a tag (the most commen tag) in case a word from the test data does not appear in the training data instead of using a random choice. Additionally, use my own previously created hmm algorithm different (and worse) from the actual viterbi algorithm.
