...
#Version 4_tokfreq: Use the token frequency to the determine a tag (the most commen tag) in case a word from the test data does not appear in the training data instead of using a random choice.
...
def get_emission_probability(train_data):
    '''Returns a dictionary representing the emission probability of all unique word-tag-pairs of a given list of word-tag-pairs *train_data*. In the final dictionary, the keys represent the words and their values, which are dictionaries, represent with their keys the (pos) tags.'''
    ...
    emis_prob_dict = dict.fromkeys(unique_words)
    for wd in emis_prob_dict:
        wd_tags_dict = dict.fromkeys(unique_tags,0)
        relevant_unique_tags = tuple(set([tag for word,tag in train_data if word == wd]))
        for tag in relevant_unique_tags:
            wd_tags_dict[tag] = get_emission_probability_base(wd,tag,train_data)
        emis_prob_dict[wd] = wd_tags_dict
...
def get_transition_probability(training_sentences):
    '''Return a dictionary representing the transition probability of all unique (pos) tags (as well as the added start- and end-sentence tags '<S>' and '<E>') of a given list *training_sentences*, whose lists contain single sentences of the training data. In the final dictionary, the keys represent the first (left) tag (of a bigram), and the values, which are dictionaries themselves, are keys representing the second (right) tag.'''
    ...
    trans_prob_dict = dict.fromkeys(unique_tags)
    for t1 in trans_prob_dict:
        unique_tags_dict = dict.fromkeys(unique_tags,0)
        for t2 in unique_tags_dict:
            if (t1 == "<E>") | (t2 == "<S>"):
                continue
            else:
                tag1_tag2,tag1_x = get_transition_probability_base(t1,t2,new_data)
                unique_tags_dict[t2] = tag1_tag2/tag1_x
        trans_prob_dict[t1] = unique_tags_dict
...
def get_most_common_tag(data):
    '''Returns given a list of word-tag-pairs the most commonly occuring (pos) tag.'''
    unique_tags = list(set([tag for wd,tag in data]))
    tag_freq = []
    for tag in unique_tags:
        freq = len([t for wd,t in data if t == tag])
...
def viterbi_hmm_algorithm(train_sentences,test_sentences):
    ...
    print(f"Training.")
    trans_prob_dict = get_transition_probability(train_sentences)
    test_sents = test_sentences
    train_data = reduce_dim(train_sentences)
    emis_prob_dict = get_emission_probability(train_data)
    print(f"Predicting.")
    ...
    for c,sent in enumerate(test_sents):
        ...
        for ind,wd_tag_pair in enumerate(sent):
            ...
            if ind == 0:
                if not tags:
                    current_paths.append(([most_common_tag],1))
                for tag in tags:
                    trans_p = trans_prob_dict["<S>"][tag]
                    emis_p = emis_prob_dict[word][tag]
                ...
                if not tags:
                    for tags_l,old_p in paths:
                        current_tags = tags_l + [most_common_tag]
                ...
                for tag in tags:
                    ...
                    emis_p = emis_prob_dict[word][tag]
                    for tags_l,old_p in paths:
                        trans_p = trans_prob_dict[tags_l[-1]][tag]
        ...
        for tags_l,old_p in paths:
            trans_p = trans_prob_dict[tags_l[-1]]["<E>"]
            if trans_p == 0:
                continue
            old_p *= trans_p
