...
#Created: 2024-02-22
#Latest Version: 2024-02-23

'''
This version of the script was created to examine why the script 'hmm_v2.py' (the most efficient of all other hmm scripts in terms of speed and memory usage) uses/allocates a lot of ram when using the tagged urum data on morphs (not on words). It turns out: The data is just bad.
'''
...
def get_csv_as_list(file_path,seperator,):
    '''Loads a csv file from *file_path* with *seperator* as the delimiter and returns it as generator with tuples, in which every tuple represents a row and every value in a tuple represents a value of a column.'''
...
#@profile
def get_sent_for_wd_tag_pairs(wd_tag_pairs,wd_end,tag_end):
    '''Returns a generator containing sentences, where a sentence is represented as a list containing any number of word-tag-pairs (as tuples) from a given list of word-tag-pairs *wd_tag_pairs* (as tuples) except for those pairs, whose word equals *wd_end* and whose tag equals *tag_end*. These pairs are only used to encode the end of a sentence and the beginning of a following sentence.'''
    tagged_sentences = []
    single_sentence = []
...
def hmm_algorithm(train_sentences,test_sentences):
    '''Creates an hmm model of *train_sentences* and predicts *test_sentences* based on that model. Returns a tuple with its first value being the accuracy of the hmm model when predicting sentences and its second value being the accuracy of the hmm model when predicting single tags (here: pos tags).'''
    ...
    for count,sent in enumerate(test_sents):
        if len(sent) > 25:
            continue
        #print(f"{count} from {len(test_sents)-1}")
        ...
        for ind,wd_tag_pair in enumerate(sent[1:]):
            word = wd_tag_pair[0]
            current_paths = []
            #print(f"{ind+1}. wd-tag-pair {wd_tag_pair}: previous combinations count: {len(paths)}")
            if ind == (len(sent[1:])-1):
                #print(f"last pair: {wd_tag_pair}")
                ...
                #DEBUG
                #print(f"second and other pairs: {wd_tag_pair}")
                #OBJECTS = [train_data,test_sents,trans_prob_df,paths,sent,current_paths,sentences_correctly_predicted,tags_correctly_predicted]
                #NAMES = ["train_data","test_sents","trans prob df","paths","sent","current_pahts","sentence_correctly_predicted","tags_correctly_predicted"]
                #if test_ram_size(OBJECTS,NAMES) != None:
                ...
                #print(f"first pair: {wd_tag_pair}")
                unique_tags,num_unique_tags = get_tag_types(word,train_data)
                if num_unique_tags == 0:
                    continue
                for tag in unique_tags:
...
def get_mb_size(object):
    x = sys.getsizeof(object)*1e-6
    return x

def test_ram_size(objects,names):
    for ind,obj in enumerate(objects):
        #if get_mb_size(obj) > 0.75:
            #print(f"{names[ind]}: mb size > 0.75")
            #return obj
        #if get_mb_size(obj) > 1:
            #print(f"{names[ind]}: mb size > 1")
            #return obj
        if get_mb_size(obj) > 10:
            print(f"{names[ind]}: mb size > 10")
            return obj
        if get_mb_size(obj) > 100:
            print(f"{names[ind]}: mb size > 100")
    return None


#ACTUAL OPERATIONS BEGIN HERE#
#tagged_words_file_path = "../data/tagged_words_urum_words.txt"
tagged_words_file_path = "../data/tagged_words_urum_morphs.txt"

#Get the word-tag-pairs from the respective file as tuples inside a lsit (generator).
...
tagged_sentences = get_sent_for_wd_tag_pairs(tagged_words_l,"<E>","<E>")
